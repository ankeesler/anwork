...
var _ = Describe("Command", func() {
	...
	Describe("summary", func() {
		BeforeEach(func() {
			...
			manager.EventsReturns([]*task2.Event{
				&task2.Event{
	...
	Describe("delete", func() {
		Context("when the manager successfully deletes the task", func() {
			BeforeEach(func() {
				...
				manager.FindByNameReturnsOnCall(0, &task2.Task{Name: "task-a"}, nil)
			})
		...
		Context("when a task spec is passed", func() {
			BeforeEach(func() {
				manager.DeleteReturnsOnCall(0, nil)
				manager.FindByIDReturnsOnCall(0, &task2.Task{Name: "task-a"}, nil)
			})
	...
	Describe("show", func() {
		...
		Context("when there are multiple tasks", func() {
			BeforeEach(func() {
				...
				manager.TasksReturnsOnCall(0, tasks, nil)
				manager.FindByNameStub = func(name string) (*task2.Task, error) {
					for _, t := range tasks {
						if t.Name == name {
							return t, nil
						}
...
State: READY`
	...
	Describe("set-<state>", func() {
		BeforeEach(func() {
			manager.FindByNameReturns(&task2.Task{Name: "task-a"}, nil)
		})
	...
	Describe("journal", func() {
		BeforeEach(func() {
			manager.EventsReturnsOnCall(0, []*task2.Event{
				&task2.Event{
					TaskID: 1,
					Title:  "event-a",
				},
		...
		Context("when task name is passed", func() {
			...
			Context("when the task does exist", func() {
				BeforeEach(func() {
					manager.FindByNameReturnsOnCall(0, &task2.Task{
						ID: 1,
					},
						nil,
					)
		...
		Context("when a task spec is passed", func() {
			BeforeEach(func() {
				manager.FindByIDReturnsOnCall(0, &task2.Task{Name: "task-a", ID: 1}, nil)
				manager.EventsReturnsOnCall(0, []*task2.Event{
					&task2.Event{
						TaskID: 1,
						Title:  "event-a",
	...
	Describe("archive", func() {
		...
		Context("when there are multiple tasks", func() {
			BeforeEach(func() {
				tasks := []*task2.Task{
					&task2.Task{Name: "task-a", State: task2.StateFinished},
					&task2.Task{Name: "task-b", State: task2.StateBlocked},
					&task2.Task{Name: "task-c", State: task2.StateFinished},
				}
				manager.TasksReturnsOnCall(0, tasks, nil)
			})
	...
	Describe("rename", func() {
		Context("the manager succeeds", func() {
			BeforeEach(func() {
				manager.FindByNameReturnsOnCall(0, &task2.Task{Name: "task-a", State: task2.StateFinished}, nil)
